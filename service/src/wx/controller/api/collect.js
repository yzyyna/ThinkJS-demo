...
module.exports = class extends Base {
  async listAction() {
    ...
    const list = await this.model('wx_collect')
  ...
  async addordeleteAction() {
    ...
    const collect = await this.model('wx_collect').where({type_id: typeId, value_id: valueId, user_id: think.userId}).find();
    let collectRes = null;
    let handleType = 'add';
    if (think.isEmpty(collect)) {
      // 添加收藏
      collectRes = await this.model('wx_collect').add({
        type_id: typeId,
        value_id: valueId,
      ...
      // 取消收藏
      collectRes = await this.model('wx_collect').where({id: collect.id}).delete();
      handleType = 'delete';
    }
