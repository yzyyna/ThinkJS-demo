...
module.exports = class extends Base {
   ...
   */
  async getCart() {
    const cartList = await this.model('wx_cart').where({user_id: think.userId, session_id: 1}).select();
    // 获取购物车统计信息
    let goodsCount = 0;
    ...
    for (const cartItem of cartList) {
      ...
      // 查找商品的图片
      cartItem.list_pic_url = await this.model('wx_goods').where({id: cartItem.goods_id}).getField('list_pic_url', true);
    }
  ...
  async addAction() {
    ...
    // 判断商品是否可以购买
    const goodsInfo = await this.model('wx_goods').where({id: goodsId}).find();
    if (think.isEmpty(goodsInfo) || goodsInfo.is_delete === 1) {
      return this.fail(400, '商品已下架');
    }

    // 取得规格的信息,判断规格库存
    const productInfo = await this.model('wx_product').where({goods_id: goodsId, id: productId}).find();
    if (think.isEmpty(productInfo) || productInfo.goods_number < number) {
      return this.fail(400, '库存不足');
    }

    // 判断购物车中是否存在此规格商品
    const cartInfo = await this.model('wx_cart').where({goods_id: goodsId, product_id: productId}).find();
    if (think.isEmpty(cartInfo)) {
      ...
      if (!think.isEmpty(productInfo.goods_specification_ids)) {
        goodsSepcifitionValue = await this.model('wx_goods_specification').where({
          goods_id: goodsId,
      ...
      await this.model('wx_cart').thenAdd(cartData, {product_id: productId});
    } else {
      // 如果已经存在购物车中，则数量增加
      ...
      await this.model('wx_cart').where({
        goods_id: goodsId,
        product_id: productId,
        id: cartInfo.id
  ...
  async updateAction() {
    ...
    const cartInfo = await this.model('wx_cart').where({id: id}).find();
    // 只是更新number
    if (cartInfo.product_id === productId) {
      await this.model('wx_cart').where({id: id}).update({
        number: number
      });
    ...
    const newCartInfo = await this.model('wx_cart').where({goods_id: goodsId, product_id: productId}).find();
    if (think.isEmpty(newCartInfo)) {
      ...
      if (!think.isEmpty(productInfo.goods_specification_ids)) {
        goodsSepcifition = await this.model('wx_goods_specification').field(['wx_goods_specification.*', 'wx_specification.name']).join('wx_specification ON wx_specification.id=wx_goods_specification.specification_id').where({
          'wx_goods_specification.goods_id': goodsId,
          'wx_goods_specification.id': {'in': productInfo.goods_specification_ids.split('_')}
        }).select();
      }
      ...
      await this.model('wx_cart').where({id: id}).update(cartData);
    } else {
      ...
      await this.model('wx_cart').where({id: newCartInfo.id}).delete();
  ...
  async checkedAction() {
    ...
    await this.model('wx_cart').where({product_id: {'in': productId}}).update({checked: parseInt(isChecked)});
  ...
  async deleteAction() {
    ...
    await this.model('wx_cart').where({product_id: {'in': productId}}).delete();
  ...
  async checkoutAction() {
    ...
    if (addressId) {
      checkedAddress = await this.model('wx_address').where({is_default: 1, user_id: think.userId}).find();
    } else {
      checkedAddress = await this.model('wx_address').where({id: addressId, user_id: think.userId}).find();
    }

    if (!think.isEmpty(checkedAddress)) {
      checkedAddress.province_name = await this.model('wx_region').getRegionName(checkedAddress.province_id);
      checkedAddress.city_name = await this.model('wx_region').getRegionName(checkedAddress.city_id);
      checkedAddress.district_name = await this.model('wx_region').getRegionName(checkedAddress.district_id);
    ...
    // 获取可用的优惠券信息，功能还示实现
    const couponList = await this.model('wx_user_coupon').select();
