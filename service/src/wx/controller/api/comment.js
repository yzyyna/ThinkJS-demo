...
module.exports = class extends Base {
  ...
  async postAction() {
    ...
    const insertId = await this.model('wx_comment').add({
      type_id: typeId,
      value_id: valueId,
  ...
  async countAction() {
    ...
    const allCount = await this.model('wx_comment').where({type_id: typeId, value_id: valueId}).count('id');

    const hasPicCount = await this.model('wx_comment').alias('comment')
      .join({
  ...
  async listAction() {
    ...
    let comments = [];
    if (showType !== 1) {
      comments = await this.model('wx_comment').where({
        type_id: typeId,
        value_id: valueId
      }).page(page, size).countSelect();
    } else {
      comments = await this.model('wx_comment').alias('comment')
        .field(['comment.*'])
        .join({
          table: 'comment_picture',
          join: 'right',
    ...
    for (const commentItem of comments.data) {
      ...
      comment.user_info = await this.model('wx_user').field(['username', 'avatar', 'nickname']).where({id: commentItem.user_id}).find();
      comment.pic_list = await this.model('wx_comment_picture').where({comment_id: commentItem.id}).select();
