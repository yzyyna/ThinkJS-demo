...
module.exports = class extends Base {
  ...
  async prepayAction() {
    ...
    const orderInfo = await this.model('wx_order').where({id: orderId}).find();
    ...
    const openid = await this.model('wx_user').where({id: orderInfo.user_id}).getField('weixin_openid', true);
  ...
  async notifyAction() {
    ...
    const orderModel = this.model('wx_order');
