     ...
     <div>
        ...
        <Modal  title="编辑"  :mask-closable="false" :closable="false" v-model="modalAdd">
            <Form ref="formRef" :model="formValidate" :rules="ruleValidate" :label-width="80">
                ...
                <FormItem label="回答" prop="answer">
                    <Input v-model="formValidate.answer"></Input>
                </FormItem>
            </Form>
            <div slot="footer">
        ...
        <Modal  title="详情" v-model="modalDetail">
            <Form :model="formValidate" :label-width="80">
                <FormItem label="问题">
                    <Input v-model="formValidate.question" readonly></Input>
                </FormItem>
                <FormItem label="回答">
                    <Input v-model="formValidate.answer" readonly></Input>
                </FormItem>
            </Form>
            <div slot="footer">
            </div>
    ...
    export default { 
        data () {
            return {
                ...
                ruleValidate: {
                    question: [
                        { required: true, message: '问题为必填项'}
                    ],
                    answer: [
                        { required: true, message: '回答为必填项'}
                    ],
                }
            }
        },
        methods: {
            ...
            init () {
                ...
                let _self=this;
                _self.loading=true;
                util.post(this,'wx/admin/wx_goods_issue/pageData',this.searchForm,function(datas){   
            ...
            remove (param) {
                let _self=this;
                this.loading=true;
                util.post(this,'wx/admin/wx_goods_issue/delData',{id:param.row.id},function(datas){ 
                ...
                this.$refs['formRef'].validate((valid) => {
                    if (valid) {
                        ...
                        if(this.formValidate&&this.formValidate.id){
                            util.post(this,'wx/admin/wx_goods_issue/updateData',_data,function(datas){  
                                _self.$Message.success('编辑成功！');
                                _self.addCanFun();
                                _self.init();      
                            });                        
                        }else{
                            util.post(this,'wx/admin/wx_goods_issue/addData',_data,function(datas){ 
